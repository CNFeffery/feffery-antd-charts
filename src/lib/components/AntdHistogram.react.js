...
const AntdHistogram = (props) => {
    return (
        <Suspense fallback={null}>
            <LazyAntdHistogram {...props} />
        </Suspense>
    );
}

// 定义参数或属性
AntdHistogram.propTypes = {
    // 部件id
    id: PropTypes.string,
    ...
    // 定义进行直方图绘制的字段名
    binField: PropTypes.string.isRequired,

    // 定义作为堆叠依据的字段名
    stackField: PropTypes.string,

    // 需要进行等距分箱时，设置对应的每个分箱宽度，不可与binNumber同时使用
    binWidth: PropTypes.number,

    // 设置直方图分箱的个数，不可与binWidth同时使用
    binNumber: PropTypes.number,
...
// 设置默认参数
AntdHistogram.defaultProps = {
    locale: 'zh-CN',
    downloadTrigger: 'download-trigger'
}

export default AntdHistogram;

export const propTypes = AntdHistogram.propTypes;
export const defaultProps = AntdHistogram.defaultProps;